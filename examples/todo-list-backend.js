...
passport.use(
  new BearerStrategy(function(token, done) {
    ...
    authContext.getObject(
      "User",
      { where: ["SELF.accessTokens.token = %s", token] },
      function(err, user) {
...
app.post("/register", function(req, res, next) {
  if (!req.body.username || !req.body.password)
    return next(new Error("username and password must be specified"));
  var password = sha512(req.body.password);
  var user = req.context.create("User", {
...
app.post("/authorize", function(req, res, next) {
  ...
  req.context
    .getObject("User", {
      where: [
        "SELF.username = %s AND SELF.password = %s",
        username,
        passwordHash
      ]
    })
...
app.use(passport.authenticate("bearer", { session: false }), function(
  req,
  res,
  next
) {
  req.context
    .getObjectWithObjectID(req.user)
    .then(function(user) {
      req.user = user;
      next();
    })
...
app.all("/me/tasks/:id", function(req, res, next) {
  req.context
    .getObject("Task", {
      where: ["SELF.id = %d AND SELF.owner = %@", req.params.id, req.user]
    })
    .then(function(task) {
      req.task = task;
      next();
