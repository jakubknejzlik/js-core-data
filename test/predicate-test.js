...
describe("Predicate", function() {
  ...
  it("should correctly format multiple strings", function() {
    ...
    assert.equal(
      predicate.toString(),
      "name = 'aa''' AND test = 'test' OR xxx = 'xxx'"
    );
  });
  it("should correctly format multiple numbers", function() {
    var predicate = new Predicate(
      "name = %d AND test = %d OR xxx = %d",
      12,
      25,
      58
    );
  ...
  it("should correctly format Objects and ObjectIDs", function() {
    ...
    assert.equal(
      predicate.toString(),
      "object_id = '[NaN]' AND objectID_id = '[NaN]'"
    );
  });
  it("should correctly format undefined values", function() {
    var predicate = new Predicate(
      "attr1 = %d AND attr2 = %s AND attr3 = %@",
      "aa",
      undefined,
      undefined
    );
    assert.equal(
      predicate.toString(),
      "attr1 = '[NaN]' AND attr2 = 'null' AND attr3_id = 0"
    );
  });
  it("should correctly format undefined arrays", function() {
    var predicate = new Predicate(
      "attr1 IN %a AND attr2 IN %a AND attr3 IN %a",
      ["a", "b"],
      [1, 2],
      ["aa", 1]
    );
    assert.equal(
      predicate.toString(),
      "attr1 IN ('a','b') AND attr2 IN (1,2) AND attr3 IN ('aa',1)"
    );
  });
  it("should correctly format NaN", function() {
    var predicate = new Predicate(
      "attr1 = %d AND attr2 != %d AND test=%s AND value > MAX(%d)",
      "hello",
      "xxx",
      "this is NaN",
      "invalid number"
    ...
    assert.equal(
      predicate.toString(),
      "attr1 = '[NaN]' AND attr2 != '[NaN]' AND test='this is NaN' AND value > MAX('[NaN]')"
    );
  });
  ...
  it("should correctly parse object condition with OR", function() {
    var predicate = new Predicate({
      $or: {
        ...
        "LEAST(test)": 24,
        testBool: true,
        $or: { nullAttr: null, "nonnullAttr!": null }
      }
    });
    assert.equal(
      predicate.toString(),
      "((SELF.tags.key IN ('aa','bb') OR LEAST(SELF.test) = 24 OR SELF.testBool = TRUE OR (SELF.nullAttr IS NULL OR SELF.nonnullAttr IS NOT NULL)))"
    );
    predicate = new Predicate({
      $or: [
        { $and: { test: "x", test2: "x2" } },
    ...
    assert.equal(
      predicate.toString(),
      "((((SELF.test = 'x' AND SELF.test2 = 'x2')) OR ((SELF.test = 'y' AND SELF.test2 = 'y2'))))"
    );
    predicate = new Predicate({
      $and: [
        { $or: { test: "x", test2: "x2" } },
    ...
    assert.equal(
      predicate.toString(),
      "((((SELF.test = 'x' OR SELF.test2 = 'x2')) AND ((SELF.test = 'y' OR SELF.test2 = 'y2'))))"
    );
  });
  it("should correctly parse object condition with AND", function() {
    var predicate = new Predicate({
      $and: {
        "SELF.tags.key": ["aa'", "bb"],
        test: 24,
        $and: { nullAttr: null, "nonnullAttr!": null }
      }
    });
    assert.equal(
      predicate.toString(),
      "((SELF.tags.key IN ('aa''','bb') AND SELF.test = 24 AND (SELF.nullAttr IS NULL AND SELF.nonnullAttr IS NOT NULL)))"
    );
  });
  ...
  it("should correctly parse object condition with custom object", function() {
    ...
    assert.equal(
      predicate.toString(),
      "(SELF.test = '" + moment(date).format("YYYY-MM-DD HH:mm:ss") + "')"
    );
  });
  it("should correctly parse object with Objects and ObjectIDs", function() {
    ...
    assert.equal(
      predicate.toString(),
      "(SELF.object_id = 1 AND SELF.objectID_id = 1)"
    );
    ...
    assert.equal(
      predicate.toString(),
      "(SELF.object_id = 2 AND SELF.objectID_id = 2)"
    );
    objectID.stringValue = "yyyy/o2";
    predicate = new Predicate({ object: object, objectID: objectID });
    assert.equal(
      predicate.toString(),
      "(SELF.object_id = '[NaN]' AND SELF.objectID_id = '[NaN]')"
    );
  });
  it("should correctly parse nested predicate object", function() {
    ...
    var predicate3 = new Predicate([
      {
        $or: {
          "SELF.followers.id": 321
        }
      },
      "EVERY(SELF.eventInvitations.id != " + 123 + ")"
    ]);
    assert.equal(
      predicate2.toString(),
      "((SELF.xx = 'xx') AND (SELF.aa = 'aa' AND SELF.bb = 'bb') AND (SELF.test = 10))"
    );
  });
});
