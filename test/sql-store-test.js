...
//    })
    ...
    it('should format SQL',function(){
        ...
        assert.equal(sql,'SELECT SELF._id AS "_id", SELF.username AS "username" FROM users `SELF` WHERE (username = \'test\') GROUP BY SELF._id ORDER BY SELF.username ASC')
    })
    ...
    it('should format SQL with joins for oneToMany',function(){
        ...
        var sql = store.sqlForFetchRequest(req);
        assert.equal(sql,"SELECT SELF._id AS \"_id\", SELF.username AS \"username\" FROM users `SELF` LEFT JOIN companies `SELF_company` ON (SELF_company._id = SELF.company_id) WHERE (SELF_company.name = 'test') GROUP BY SELF._id ORDER BY SELF_company.name ASC")
    })
    ...
    it('should format SQL with joins for manyToMany',function(){
        ...
        var sql = store.sqlForFetchRequest(req);
        assert.equal(sql,"SELECT SELF._id AS \"_id\", SELF.username AS \"username\" FROM users `SELF` LEFT JOIN teams_users `SELF_teams__mid` ON (SELF._id = SELF_teams__mid.users_id) LEFT JOIN teams `SELF_teams` ON (SELF_teams__mid.reflexive = SELF_teams._id) WHERE (SELF_teams.name = 'test') GROUP BY SELF._id ORDER BY SELF_teams.name ASC")
