...
coreData.createModelFromYaml(
  fs.readFileSync(__dirname + '/schemes/car-model.yaml'),
  ...
  describe('store stuff', function() {
    it('should throw error when creating coordinator with null object model', function() {
      assert.throws(
        ...
        err => err.message == 'Cannot create coordinator without object model'
      );
    });
  });
  ...
  describe('context', function() {
    ...
    describe('object creation', function() {
      ...
      it('should set toOne relation', function(done) {
        ...
        context
          ...
          .then(function() {
            return tempContext
              .getObjectWithObjectID(car.objectID)
              .then(function(tempCar) {
    ...
    describe('fetching', function() {
      ...
      it('should load offest objects', function(done) {
        context.getObjects('Car', { offset: 1, limit: 2 }, function(
          err,
          objects
        ) {
          assert.ifError(err);
      ...
      it("should load object by attribute(lastName = 'test2')", function(done) {
        context.getObjects(
          ...
          {
            where: [
              'SELF.lastName = %s OR SELF.lastName = %s',
              'test2',
              'SELF.lastName'
            ]
          },
      ...
      it('should load same objects(uniquely)', function(done) {
        ...
        context.getObjects('Car', function(err, objs) {
          ...
          for (var i = 0; i < count; i++) {
            context.getObjectWithId(
              obj.entity.name,
              obj.objectID.recordId(),
              function(err, object) {
                objects.push(object);
      ...
      it('should load same objects from relation(uniquely)', function(done) {
        ...
        context.save(function(err) {
          ...
          _context.getObjects('Car', function(err, _cars) {
            assert.ifError(err);
            _context.getObjectWithObjectID(owner.objectID, function(
              err,
              _owner
            ) {
              assert.ifError(err);
              _owner
                .getCars()
                .then(function(__cars) {
                  assert.ok(__cars.length > 0, 'object count');
                  var _car = __cars[0];
                  assert.ifError(err);
                  _cars.forEach(function(_c) {
                    assert.ok(
                      _car.objectID.toString() != _c.objectID.toString() ||
                        _car === _c,
                      _car.objectID.toString() +
                        ' == ' +
                        _c.objectID.toString() +
        ...
        it('should load relation object after saving', function(done) {
          context2.getObjectWithObjectID(car.objectID, function(err, _car) {
            ...
            _car.getOwner(function(err, _owner) {
              ...
              assert.ok(_owner);
              assert.equal(
                owner._objectID.toString(),
                _owner._objectID.toString()
              );
              done();
            });
          });
      ...
      describe('manyToMany', function() {
        ...
        it('should get assigned object', function(done) {
          ...
          owner.getVisitedCars(function(err, visitedCars) {
            ...
            ids = [];
            if (visitedCars)
              visitedCars.forEach(function(vc) {
        ...
        it('should add object to many-2-many repeatedle', function(done) {
          async.timesSeries(
            ...
            function(i, cb) {
              var context2 = coreData.createContext();
              context2
                .getObjectWithObjectID(owner.objectID)
      ...
      describe('oneToOne', function() {
        ...
        it('should load assigned one to one objects', function(done) {
          ...
          context2.getObjects('Seller', function(err, sellers) {
            ...
            assert.equal(
              _seller.objectID.toString(),
              seller.objectID.toString()
            );
            _seller.getLicence(function(err, _licence) {
              assert.ifError(err);
              assert.equal(
                _licence.objectID.toString(),
                licence.objectID.toString()
              );
              done();
            });
          });
