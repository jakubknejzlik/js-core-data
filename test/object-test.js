...
(moment = require("moment")), (CoreData = require("../")), (fs = require("fs"));
...
describe("ManagedObject", function() {
  describe("attributes", function() {
    ...
    ModelYamlParser.fillModelFromYaml(
      objectModel,
      fs.readFileSync(__dirname + "/schemes/object-test-model.yaml"),
      { Hello: require("./Classes/Hello") }
    );
    ...
    it("should throw error for invalid model", function() {
      assert.throws(function() {
        ...
        ModelYamlParser.fillModelFromYaml(
          objectModel,
          fs.readFileSync(
            __dirname + "/schemes/attribute-invalid-test-model.yaml"
          )
        );
    ...
    describe("validation", function() {
      ...
      before(function(done) {
        ...
        storeCoordinator.persistentStores[0]
      ...
      it("should load all attributes", function(done) {
        ...
        context.getObjects("Hello", function(err, objects) {
          ...
          assert.equal(
            JSON.stringify(obj.transformable),
            JSON.stringify(transformableObject)
          );
          assert.equal(
            JSON.stringify(obj.transformableArray),
            JSON.stringify(transformableArray)
          );
          assert.equal(obj.getWorldID(), null);
          done();
        });
      ...
      it("should load null attributes", function(done) {
        ...
        context
          ...
          .then(function() {
            context
              .getObject("Hello", { where: ["SELF.bool IS NULL"] })
              .then(function(obj) {
                assert.strictEqual(obj.bool, null, "bool value");
                assert.strictEqual(obj.name, null);
                assert.strictEqual(obj.int, null);
      ...
      it("should pass valid url", function() {
        ...
        assert.doesNotThrow(function() {
          obj.setValues({
            url: "ftp://www.google.com/test/com.abcd?testmy&arg2=a"
          });
        });
        assert.doesNotThrow(function() {
      ...
      it("private attribute", function() {
        ...
        assert.equal(
          obj.getValues({ privates: true }).privateAttribute,
          "new Value"
        );
      ...
      it("should persist/load private attributes", function(done) {
        ...
        context
          ...
          .then(function() {
            return context2
              .getObjectWithObjectID(obj.objectID)
              .then(function(obj2) {
