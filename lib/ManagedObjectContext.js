...
(function() {
  var FetchRequest, ManagedObject, ManagedObjectContext, ManagedObjectID, Predicate, RelationshipDescription, ac, async,
  ...
  ManagedObjectContext = (function(superClass) {
    ...
    ManagedObjectContext.prototype.getObject = function(entityName, predicate, callback) {
      return this.getObjects(entityName, predicate, null, function(err, objects) {
        if (err) {
          return callback(err);
        }
        if (objects.length > 0) {
            ...
            case RelationshipDescription.deleteRules.NULLIFY:
              return _this._getObjectsForRelationship(relationship, object, _this, function(err, objects) {
                ...
                if (objects) {
                  for (i = 0, len = objects.length; i < len; i++) {
                    obj = objects[i];
                    object._removeObjectFromRelation(obj, relationship, relationship.inverseRelationship(), true);
                    obj._removeObjectFromRelation(object, relationship.inverseRelationship(), relationship, true);
                  }
                }
                return cb();
              });
