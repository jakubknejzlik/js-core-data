...
(function() {
  var AttributeTransformer, AttributeValidator, ManagedObject, ManagedObjectID, Q, RelationshipDescription, _, ac, async, capitalizedString,
  ...
  ManagedObject = (function(superClass) {
    ...
    ManagedObject.prototype.fetchData = function() {
      var attributeDescription, data, i, j, len, len1, ref, ref1, relationship;
      data = {};
      if (this._rawData) {
        ref = this.entity.attributes;
        for (i = 0, len = ref.length; i < len; i++) {
          attributeDescription = ref[i];
          data[attributeDescription.name] = AttributeTransformer.transformedValueForAttribute(this._rawData[attributeDescription.name], attributeDescription);
        }
    ...
    ManagedObject.prototype.setValues = function(values, allowedAttributes) {
      var attributeDescription, i, len, ref, ref1, results, setterFnName;
      ref = this.entity.attributes;
      results = [];
      for (i = 0, len = ref.length; i < len; i++) {
        attributeDescription = ref[i];
        setterFnName = 'set' + capitalizedString(attributeDescription.name);
        if ((values[attributeDescription.name] != null) && (!allowedAttributes || (ref1 = attributeDescription.name, indexOf.call(allowedAttributes, ref1) >= 0))) {
    ...
    ManagedObject.prototype.getValues = function(options) {
      var attributeDescription, getterFnName, i, j, len, len1, ref, ref1, relationship, value, values;
      if (options == null) {
        options = {};
      }
      if (this.isFault) {
    ...
    ManagedObject.prototype._didUpdateValues = function() {
      var ref;
      if (this.managedObjectContext.locked) {
