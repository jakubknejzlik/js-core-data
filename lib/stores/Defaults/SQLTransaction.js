...
(function() {
  ...
  SQLTransaction = (function() {
    ...
    SQLTransaction.prototype.query = function(query, params, callback) {
      ...
      if (this.rollbacked) {
        return callback(
          new Error("cannot send query to rolled back transaction")
        );
      }
    ...
    SQLTransaction.prototype.commit = function(callback) {
      ...
      if (this.committed) {
        return callback(
          new Error("cannot commit already commited transaction")
        );
      }
      if (this.rollbacked) {
    ...
    SQLTransaction.prototype.rollback = function(callback) {
      ...
      if (this.rollbacked) {
        return callback(
          new Error("cannot rollback already rolled back transaction")
        );
      }
    ...
    SQLTransaction.prototype.createRow = function(tableName, id, callback) {
      callback = this._wrapCallbackForAutoRollback(callback);
      return this.ensureBegin(
        (function(_this) {
          return function(err) {
            ...
            return _this.connection.createRow(tableName, id, callback);
          };
        })(this)
      );
    };
