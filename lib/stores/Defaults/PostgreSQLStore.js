...
try {
  ...
  e = error;
  throw new Error(
    "pg module is required to use SQLite storage, please install it by running npm install --save pg"
  );
}
...
if (process.env.NODE_ENV === "production") {
  try {
    ...
    e = error1;
    console.log(
      "pg-native is recommended for running in production environment, you install module by running  npm install --save pg-native"
    );
  }
}
...
PostgreSQLStore = (function(superClass) {
  ...
  PostgreSQLStore.prototype.createSchemaQueries = function(
    options,
    transaction,
    callback
  ) {
            ...
            for (key in ref2) {
              ...
              for (j = 0, len1 = ref3.length; j < len1; j++) {
                ...
                if (!relationship.toMany) {
                  sqls.push(
                    'ALTER TABLE "' +
                      _this._formatTableName(entity.name) +
                      '" ADD CONSTRAINT "fk_' +
                      _this._formatTableName(entity.name) +
                      "_" +
                      relationship.name +
  ...
  PostgreSQLStore.prototype.createEntityQueries = function(
    entity,
    force,
    options
  ) {
  ...
  PostgreSQLStore.prototype._relationshipColumnDefinition = function(
    relationship
  ) {
    return '"' + relationship.name + '_id" int DEFAULT NULL';
  };

  PostgreSQLStore.prototype._insertQueryForManyToMany = function(
    relationship,
    object,
    addedObject
  ) {
  ...
  PostgreSQLStore.prototype._addRelationshipQueries = function(
    entityName,
    relationship
  ) {
    return [
  ...
  PostgreSQLStore.prototype.createRelationshipQueries = function(
    relationship,
    force
  ) {
      ...
      if (inversedRelationship.toMany) {
        ...
        parts.push(
          'CONSTRAINT "fk_' +
            ...
            '" FOREIGN KEY ("reflexive") REFERENCES "' +
            this._formatTableName(reflexiveRelationship.entity.name) +
            '"("' +
            ManagedObjectID.idColumnName +
            '") ON DELETE CASCADE'
  ...
  PostgreSQLStore.prototype.columnTypeForAttribute = function(attribute) {
    switch (attribute.persistentType) {
      ...
      case "data":
        return "bytea";
      default:
        return PostgreSQLStore.__super__.columnTypeForAttribute.call(
          this,
          attribute
        );
    }
  ...
  PostgreSQLStore.prototype.encodeValueForAttribute = function(
    value,
    attribute
  ) {
    switch (attribute.persistentType) {
    ...
    return PostgreSQLStore.__super__.encodeValueForAttribute.call(
      this,
      value,
      attribute
    );
...
PostgreSQLConnection = (function(superClass) {
  ...
  PostgreSQLConnection.prototype.createRow = function(tableName, id, callback) {
    let idValue = id || "DEFAULT";
    let query = `INSERT INTO "${tableName}" ("${ManagedObjectID.idColumnName}") VALUES (${idValue}) RETURNING "${ManagedObjectID.idColumnName}"`;
    return this.connection.query(query, function(err, result) {
      if (err) {
        return callback(err);
      }
      return callback(null, result.rows[0][ManagedObjectID.idColumnName]);
    });
  };
