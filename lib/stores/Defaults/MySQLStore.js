...
try {
  ...
  e = error;
  throw new Error(
    'mysql module is required to use MySQL storage, please install it by running npm install --save mysql'
  );
}
...
MySQLStore = (function(superClass) {
  ...
  MySQLStore.prototype.createSchemaQueries = function(
    options,
    transaction,
    callback
  ) {
        ...
        return function(err, rows) {
          ...
          return async.forEach(
            ...
            function(tableName, cb) {
              var fksQuery;
              fksQuery =
                "SELECT CONSTRAINT_NAME as constraint_name FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE where TABLE_NAME = '" +
                tableName +
            ...
            function(err) {
              var entity, error1, j, key, len1, ref, ref1, tableName;
              if (err) {
                return callback(err);
              }
              try {
                ...
                sqls.push(
                  'CREATE TABLE IF NOT EXISTS `_meta` (`key` varchar(10) NOT NULL,`value` varchar(250) NOT NULL,PRIMARY KEY (`key`)) ENGINE=InnoDB  DEFAULT CHARSET=utf8'
                );
                sqls.push(
                  "INSERT INTO `_meta` VALUES('version','" +
                    objectModel.version +
  ...
  MySQLStore.prototype.createEntityQueries = function(entity, force, options) {
    ...
    parts = [
      '`' + ManagedObjectID.idColumnName + '` int(11) NOT NULL AUTO_INCREMENT',
      'PRIMARY KEY (`' + ManagedObjectID.idColumnName + '`)'
    ];
  ...
  MySQLStore.prototype._foreignKeyNameForRelationship = function(relationship) {
    return (
      'fk_' +
      this._formatTableName(relationship.entity.name) +
      '_' +
      relationship.name +
      '_id'
    );
  ...
  MySQLStore.prototype._foreignKeyDefinitionForRelationship = function(
    relationship
  ) {
    return (
      'CONSTRAINT `' +
      this._foreignKeyNameForRelationship(relationship) +
      '` FOREIGN KEY (`' +
      relationship.name +
      '_id`) REFERENCES `' +
  ...
  MySQLStore.prototype._renameRelationshipQuery = function(
    tableName,
    relationshipFrom,
    relationshipTo
  ) {
    ...
    sqls.push(
      'ALTER TABLE ' +
        ...
        this.quoteSymbol +
        ' DROP FOREIGN KEY `' +
        this._foreignKeyNameForRelationship(relationshipFrom) +
        '`'
    );
    sqls.push(
      'ALTER TABLE ' +
    ...
    sqls.push(
      'ALTER TABLE ' +
        ...
        this.quoteSymbol +
        ' ADD ' +
        this._foreignKeyDefinitionForRelationship(relationshipTo)
    );
    return sqls.join(';');
  };

  MySQLStore.prototype._renameAttributeQuery = function(
    tableName,
    attributeFrom,
    attributeTo
  ) {
    return (
      ...
      this.quoteSymbol +
      ' ' +
      this._columnDefinitionForAttribute(attributeTo)
    );
  };

  MySQLStore.prototype._removeRelationshipQuery = function(
    entityName,
    relationship
  ) {
    var columnName,
    ...
    if (relationship.toMany && inverseRelationship.toMany) {
      ...
      return (
        ...
        this.quoteSymbol +
        ';' +
        this._removeColumnQuery(entityName, columnName)
      );
    }
  };

  MySQLStore.prototype._relationshipColumnDefinition = function(relationship) {
    return MySQLStore.__super__._relationshipColumnDefinition.call(
      this,
      relationship
    );
  };

  MySQLStore.prototype._addRelationshipQueries = function(
    entityName,
    relationship
  ) {
    return [
      'ALTER TABLE ' +
        ...
        '_id' +
        this.quoteSymbol +
        ' int(11) DEFAULT NULL, ADD ' +
        this._foreignKeyDefinitionForRelationship(relationship)
    ];
  };

  MySQLStore.prototype._insertQueryForManyToMany = function(
    relationship,
    object,
    addedObject
  ) {
    return (
      ...
      ') ON DUPLICATE KEY UPDATE `reflexive` = VALUES(`reflexive`)'
    );
  };

  MySQLStore.prototype.createRelationshipQueries = function(
    relationship,
    force
  ) {
      ...
      if (inversedRelationship.toMany) {
        ...
        parts.push(
          'CONSTRAINT `fk_' +
            ...
            '_' +
            reflexiveRelationship.inverseRelationship().name +
            '_reflexive_id` FOREIGN KEY (`reflexive`) REFERENCES `' +
            this._formatTableName(reflexiveRelationship.entity.name) +
            '`(`' +
            ManagedObjectID.idColumnName +
            '`) ON DELETE CASCADE'
  ...
  MySQLStore.prototype.columnTypeForAttribute = function(attribute) {
    ...
    switch (attribute.persistentType) {
      ...
      default:
        return MySQLStore.__super__.columnTypeForAttribute.call(
          this,
          attribute
        );
    }
  ...
  MySQLStore.prototype.encodeValueForAttribute = function(value, attribute) {
    switch (attribute.persistentType) {
      case 'date':
        if (value === null) {
          return null;
    ...
    return MySQLStore.__super__.encodeValueForAttribute.call(
      this,
      value,
      attribute
    );
...
MySQLConnection = (function(superClass) {
  ...
  MySQLConnection.prototype.createRow = function(tableName, id, callback) {
    let idValue = id || 'NULL';
