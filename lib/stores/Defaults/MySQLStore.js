...
(function() {
  var GenericPool, GenericSQLStore, ManagedObjectID, MySQLConnection, MySQLStore, PersistentStoreRequest, Predicate, SQLConnection, String, _, async, e, error, mysql,
  ...
  MySQLStore = (function(superClass) {
    ...
    MySQLStore.prototype.createSchemaQueries = function(options) {
      var entity, key, objectModel, ref, ref1, sqls;
      if (options == null) {
        options = {};
      }
      ...
      for (key in ref1) {
        entity = ref1[key];
        sqls = sqls.concat(this.createEntityRelationshipQueries(entity, options.force));
      }
    ...
    MySQLStore.prototype.createEntityQueries = function(entity, force, options) {
      ...
      if (force) {
        sqls = sqls.concat(this._dropEntityQueries(entity));
      }
    ...
    MySQLStore.prototype._addRelationshipQueries = function(entityName, relationship) {
      return ['ALTER TABLE ' + this.quoteSymbol + this._formatTableName(entityName) + this.quoteSymbol + ' ADD COLUMN ' + this._relationshipColumnDefinition(relationship) + ', ADD ' + this._foreignKeyDefinitionForRelationship(relationship)];
    };
    ...
    MySQLStore.prototype.createRelationshipQueries = function(relationship, force) {
      ...
      if (relationship.toMany) {
        ...
        if (inversedRelationship.toMany) {
          ...
          parts.push('CONSTRAINT `fk_' + this._formatTableName(reflexiveRelationship.destinationEntity.name) + '_' + reflexiveRelationship.name + '_id` FOREIGN KEY (`' + reflexiveRelationship.name + '_id`) REFERENCES `' + this._formatTableName(reflexiveRelationship.destinationEntity.name) + '`(`_id`) ON DELETE CASCADE');
          parts.push('CONSTRAINT `fk_' + this._formatTableName(reflexiveRelationship.entity.name) + '_' + reflexiveRelationship.inverseRelationship().name + '` FOREIGN KEY (`reflexive`) REFERENCES `' + this._formatTableName(reflexiveRelationship.entity.name) + '`(`_id`) ON DELETE CASCADE');
