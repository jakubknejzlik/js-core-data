      ...
      @connection.createTransaction (transaction)=>
        ...
        async.series [
          ...
          (insertedObject,cb)=>
            ...
            updates = []
            for key,value of values
              updates.push('`' + key + '` = ' + sqlite.escape(value))
            if updates.length > 0
              sql = 'UPDATE `' + formattedTableName + '` SET ' + updates.join(',') + ' WHERE `_id` = ' + sqlite.escape(id);
              transaction.sendQuery sql,(err,result)=>
                if err
                  return cb(err)
          ...
          (deletedObject,cb)=>
            ...
            sql = 'DELETE FROM `' + formattedTableName + '` WHERE `_id` = ' + sqlite.escape(id)
            transaction.sendQuery sql,(err)->
              cb(err)
          ,(err)=>
            seriesCallback(err)
...
#          console.log('xxxxx',object.relationChanges);
            sql = 'INSERT INTO `' + @_formatTableName(relationship.entity.name) + '_' + relationship.name + '` SET reflexive = ' + sqlite.escape(@_recordIDForObjectID(object.objectID)) + ',`' + relationship.name + '_id` = ' + sqlite.escape(@_recordIDForObjectID(addedObject.objectID))
            sqls.push(sql)
...
#          console.log('xxxxx',object.relationChanges);
            sql = 'DELETE FROM `' + @_formatTableName(relationship.entity.name) + '_' + relationship.name + '` WHERE reflexive = ' + sqlite.escape(@_recordIDForObjectID(object.objectID)) + ' AND `' + relationship.name + '_id` = ' + sqlite.escape(@_recordIDForObjectID(removedObject.objectID))
            sqls.push(sql)
  ...
  valuesForRelationship:(relationship,ObjectID,context,callback) ->
    ...
    if relationship.toMany and inversedRelationship.toMany
      if mainRelationship is relationship
        sql = 'SELECT ' + columns.join(',') + ' FROM `' + @_formatTableName(relationship.destinationEntity.name) + '` WHERE `_id` IN (SELECT `' + relationship.name + '_id` FROM `' + @_formatTableName(relationship.entity.name) + '_' + relationship.name + '` WHERE `reflexive` = ' + sqlite.escape(@_recordIDForObjectID(ObjectID)) + ')'
      else
        sql = 'SELECT ' + columns.join(',') + ' FROM `' + @_formatTableName(relationship.destinationEntity.name) + '` WHERE `_id` IN (SELECT `reflexive` FROM `' + @_formatTableName(inversedRelationship.entity.name) + '_' + inversedRelationship.name + '` WHERE `' + inversedRelationship.name + '_id` = ' + sqlite.escape(@_recordIDForObjectID(ObjectID)) + ')'
#      console.log('sql!!??',relationship.name,sql)
    else
#      if mainRelationship is relationship
      if inversedRelationship.toMany
        sql = 'SELECT ' + columns.join(',') + ' FROM `' + @_formatTableName(relationship.destinationEntity.name) + '` WHERE `_id` IN (SELECT `' + relationship.name + '_id` FROM `' + @_formatTableName(relationship.entity.name) + '` WHERE `_id` = ' + sqlite.escape(@_recordIDForObjectID(ObjectID)) + ') LIMIT 1'
      else
        sql = 'SELECT ' + columns.join(',') + ' FROM `' + @_formatTableName(relationship.destinationEntity.name) + '` WHERE `' + inversedRelationship.name + '_id` = ' + sqlite.escape(@_recordIDForObjectID(ObjectID))
    #      else
...
class SQLiteConnection extends Object
  constructor: (url,@store,settings)->
    @pool = GenericPool.Pool({
      name     : "sqlite",
      create   : (callback)->
        connection = sqlite.createConnection(url)
