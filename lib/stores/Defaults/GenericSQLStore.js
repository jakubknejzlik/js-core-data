...
(function() {
  var FetchRequest, GenericSQLStore, IncrementalStore, ManagedObjectID, PersistentStoreRequest, Predicate, SQLConnectionPool, SortDescriptor, _, async, squel,
  ...
  GenericSQLStore = (function(superClass) {
    ...
    GenericSQLStore.prototype.columnTypeForAttribute = function(attribute) {
      ...
      switch (attribute.persistentType) {
        ...
        case 'email':
        case 'url':
          return type = 'varchar(' + (attribute.info.length || 255) + ')';
        case 'text':
          if (attribute.info.length) {
            if (attribute.info.length < 256) {
              return type = 'tinytext';
        ...
        case 'int':
        case 'integer':
          return type = 'int(' + (attribute.info.length || 11) + ')';
        case 'decimal':
          return type = 'decimal(' + (attribute.info.digits || 20) + ',' + (attribute.info.decimals || 5) + ')';
        case 'float':
          return type = 'float';
        case 'double':
          return type = 'double';
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        switch (change.change) {
          ...
          case '-':
            ...
            break;
          default:
            sqls.push('ALTER TABLE ' + this.quoteSymbol + this._formatTableName(change) + this.quoteSymbol + ' RENAME TO ' + this.quoteSymbol + this._formatTableName(entityName) + this.quoteSymbol);
        }
      }
