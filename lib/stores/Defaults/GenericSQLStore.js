...
(function() {
  ...
  GenericSQLStore = (function(superClass) {
    ...
    GenericSQLStore.prototype.createMigrationQueries = function(migration) {
      var addedEntitiesNames, attribute, change, e, entityChangedNames, entityFrom, entityName, entityTo, error, error1, inverseRelationship, j, k, l, len, len1, len10, len11, len12, len2, len3, len4, len5, len6, len7, len8, len9, m, modelFrom, modelTo, n, newAttribute, newInverseRelationship, newReflexiveRelationship, newReflexiveTableName, newRelationship, o, p, q, r, ref, ref1, ref10, ref11, ref12, ref13, ref14, ref15, ref16, ref17, ref18, ref19, ref2, ref20, ref21, ref22, ref23, ref24, ref25, ref26, ref3, ref4, ref5, ref6, ref7, ref8, ref9, reflexiveRelationship, reflexiveTableName, relationship, s, sqls, t, u, updatedEntities, v;
      sqls = [];
      entityChangedNames = {};
      addedEntitiesNames = [];
          ...
          case '+':
            ...
            for (k = 0, len1 = ref1.length; k < len1; k++) {
              ...
              if (!inverseRelationship.toMany) {
                sqls = sqls.concat(this._addRelationshipQueries(inverseRelationship.entity.name, inverseRelationship));
              }
            }
            break;
          case '-':
      ...
      ref2 = migration.entitiesChanges;
      for (l = 0, len2 = ref2.length; l < len2; l++) {
        change = ref2[l];
        entityName = change.entity;
        switch (change.change) {
          case '+':
            ref3 = modelTo.getEntity(entityName).relationships;
      ...
      for (n = 0, len4 = updatedEntities.length; n < len4; n++) {
        ...
        if (entityFrom) {
          ref4 = entityFrom.getNonTransientAttributes();
        ...
        if (entityTo && indexOf.call(addedEntitiesNames, entityName) < 0) {
          ref6 = entityTo.getNonTransientAttributes();
          for (p = 0, len6 = ref6.length; p < len6; p++) {
            attribute = ref6[p];
            change = (ref7 = migration.attributesChanges[entityName]) != null ? ref7[attribute.name] : void 0;
            if (change === '+') {
        ...
        if (entityFrom) {
          ...
          for (q = 0, len7 = ref8.length; q < len7; q++) {
            relationship = ref8[q];
            if (!relationship.toMany) {
              change = (ref9 = migration.relationshipsChanges[entityName]) != null ? ref9[relationship.name] : void 0;
        ...
        if (entityFrom) {
          ...
          for (r = 0, len8 = ref10.length; r < len8; r++) {
            ...
            if (relationship.toMany && inverseRelationship.toMany) {
              change = ((ref11 = migration.relationshipsChanges[entityName]) != null ? ref11[relationship.name] : void 0) || ((ref12 = migration.relationshipsChanges[inverseRelationship.entity.name]) != null ? ref12[inverseRelationship.name] : void 0);
              if (change) {
                switch (change) {
                  case '+':
                    break;
          ...
          for (s = 0, len9 = ref13.length; s < len9; s++) {
            relationship = ref13[s];
            inverseRelationship = relationship.inverseRelationship();
            reflexiveRelationship = this._relationshipByPriority(relationship, inverseRelationship);
            reflexiveTableName = this._formatTableName(reflexiveRelationship.entity.name) + '_' + reflexiveRelationship.name;
            if (relationship.toMany && inverseRelationship.toMany) {
              change = (ref14 = migration.relationshipsChanges[entityName]) != null ? ref14[relationship.name] : void 0;
              if (change && (change !== '+' && change !== '-')) {
                newRelationship = entityTo.getRelationship(change);
      ...
      ref15 = modelFrom.entities;
      for (entityName in ref15) {
        entityFrom = ref15[entityName];
        ref16 = entityFrom.relationships;
        for (t = 0, len10 = ref16.length; t < len10; t++) {
          relationship = ref16[t];
          inverseRelationship = relationship.inverseRelationship();
          if (!relationship.toMany) {
            change = ((ref17 = migration.relationshipsChanges[entityName]) != null ? ref17[relationship.name] : void 0) || ((ref18 = migration.relationshipsChanges[inverseRelationship.entity.name]) != null ? ref18[inverseRelationship.name] : void 0);
            if (change) {
              switch (change) {
                case '+':
                  break;
      ...
      ref19 = modelTo.entities;
      for (entityName in ref19) {
        entityTo = ref19[entityName];
        if (entityTo && (ref20 = entityTo.name, indexOf.call(addedEntitiesNames, ref20) < 0)) {
          ref21 = entityTo.relationships;
          for (u = 0, len11 = ref21.length; u < len11; u++) {
            relationship = ref21[u];
            inverseRelationship = relationship.inverseRelationship();
            if (!relationship.toMany) {
        ...
        ref24 = entityTo.relationships;
        for (v = 0, len12 = ref24.length; v < len12; v++) {
          relationship = ref24[v];
          inverseRelationship = relationship.inverseRelationship();
          if (relationship.toMany && inverseRelationship.toMany) {
            change = ((ref25 = migration.relationshipsChanges[entityName]) != null ? ref25[relationship.name] : void 0) || ((ref26 = migration.relationshipsChanges[inverseRelationship.entity.name]) != null ? ref26[inverseRelationship.name] : void 0);
            if (change === '+') {
              sqls = sqls.concat(this.createRelationshipQueries(relationship));
            }
          }
