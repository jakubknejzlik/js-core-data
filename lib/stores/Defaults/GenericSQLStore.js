...
(function() {
  var GenericPool, GenericSQLStore, IncrementalStore, ManagedObjectID, PersistentStoreRequest, Predicate, _, async, squel,
  ...
  GenericSQLStore = (function(superClass) {
    ...
    function GenericSQLStore(storeCoordinator, URL) {
      ...
      if (request.type === 'fetch') {
        return this.connection.sendRawQuery(this.sqlForFetchRequest(request), (function(_this) {
          return function(err, rows) {
            var _row, attribute, ids, j, k, len, len1, objectID, ref, row;
            ids = [];
            if (err) {
              return callback(err);
            }
    ...
    GenericSQLStore.prototype.sqlForFetchRequest = function(request) {
      var attribute, column, descriptor, descriptors, j, k, len, len1, query, ref;
      query = squel.select().from(this._formatTableName(request.entity.name), this.tableAlias).field(this.tableAlias + '._id', '_id');
      ref = request.entity.attributes;
      ...
      if (Array.isArray(request.sortDescriptors) && request.sortDescriptors.length > 0) {
        ...
        for (k = 0, len1 = descriptors.length; k < len1; k++) {
    ...
    GenericSQLStore.prototype._getRawTranslatedQueryWithJoins = function(query, request) {
      ...
      leftJoin = (function(_this) {
        return function(subkeys, parentEntity, path) {
          var as, inversedRelation, middleTableName, middleTableNameAlias, parentAlias, pathAlias, reflexiveRelation, relation, subPath;
          as = subkeys.shift();
          ...
          if (!~alreadyJoined.indexOf(subPath)) {
            ...
            if (relation.toMany && inversedRelation.toMany) {
              reflexiveRelation = _this._relationshipByPriority(relation, inversedRelation);
              inversedRelation = relation.inverseRelationship();
              middleTableName = _this._getMiddleTableNameForManyToManyRelation(reflexiveRelation);
              ...
              query.left_join(_this._formatTableName(reflexiveRelation.destinationEntity.name), pathAlias, middleTableNameAlias + ".reflexive" + " = " + pathAlias + "._id");
            } else {
              if (relation.toMany) {
    ...
    GenericSQLStore.prototype.valuesForObject = function(ObjectID, context) {
      return this.fetchedObjectValuesCache[ObjectID.toString()] || {};
    };

    GenericSQLStore.prototype.valuesForRelationship = function(relationship, ObjectID, context, callback) {
      var attribute, columns, inversedRelationship, j, len, mainRelationship, ref, sql;
      inversedRelationship = relationship.inverseRelationship();
      sql = null;
      columns = ['`_id` as _id'];
      ...
      return this.connection.sendRawQuery(sql, (function(_this) {
        return function(err, rows) {
          var _row, ids, k, l, len1, len2, objectID, ref1, row;
          ids = [];
          if (err) {
            return callback(err);
          }
