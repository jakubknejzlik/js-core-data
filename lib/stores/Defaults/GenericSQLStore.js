...
GenericSQLStore = (function(superClass) {
  ...
  GenericSQLStore.prototype.execute = function(
    request,
    context,
    callback,
    afterInsertCallback
    ...
    if (!(request instanceof PersistentStoreRequest)) {
      throw new Error(
        "request " + request + " is not instance of PersistentStoreRequest"
      );
    }
    if (request.type === "save") {
      this.connectionPool.createTransaction(
        (function(_this) {
            ...
            return _async.series(
              [
                function(seriesCallback) {
                  return _async.forEach(
                    request.insertedObjects,
                    function(insertedObject, cb) {
                      var formattedTableName;
                      formattedTableName = _this._formatTableName(
                        insertedObject.entity.name
                      );

                      let id = insertedObject._objectID.isTemporaryID
                        ? null
                        : insertedObject.id;

                      return transaction.createRow(
                        formattedTableName,
                        id,
                        function(err, rowId) {
                          if (err) {
                          ...
                          _this.permanentIDsCache[
                            insertedObject.objectID.toString()
                          ] = rowId;
                          return cb();
                        }
                      );
                ...
                function(seriesCallback) {
                  return _async.forEach(
                    request.deletedObjects,
                    function(deletedObject, cb) {
                      var formattedTableName, id, sql;
                      formattedTableName = _this._formatTableName(
                        deletedObject.entity.name
                      );
                ...
                function(seriesCallback) {
                  return _async.forEachSeries(
                    request.insertedObjects,
                    function(insertedObject, cb) {
                      var ref, sql, updateValues;
                      (ref = _this.updateQueryForUpdatedObject(insertedObject)),
                        (sql = ref[0]),
                        (updateValues = ref[1]);
                      if (sql) {
                ...
                function(seriesCallback) {
                  return _async.forEachSeries(
                    request.insertedObjects,
                    function(insertedObject, cb) {
                      return _this._updateRelationsForObject(
                        transaction,
                        insertedObject,
                        cb
                      );
                ...
                function(seriesCallback) {
                  return _async.forEachSeries(
                    ...
                    function(updatedObject, cb) {
                      var ref, sql, updateValues;
                      (ref = _this.updateQueryForUpdatedObject(updatedObject)),
                        (sql = ref[0]),
                        (updateValues = ref[1]);
                      if (sql) {
                        return transaction.query(sql, updateValues, cb);
    ...
    if (request.type === "fetch") {
      return this.connectionPool.query(
        this.sqlForFetchRequest(request),
        (function(_this) {
          return function(err, rows) {
            ...
            for (j = 0, len = rows.length; j < len; j++) {
              ...
              objectID = _this._permanentIDForRecord(
                request.entity,
                row[ManagedObjectID.idColumnName]
              );
              ref2 = request.entity.getNonTransientAttributes();
              for (m = 0, len3 = ref2.length; m < len3; m++) {
                attribute = ref2[m];
                _row[attribute.name] = _this.decodeValueForAttribute(
                  _row[attribute.name],
                  attribute
                );
              }
  ...
  GenericSQLStore.prototype.numberOfObjectsForFetchRequest = function(
    request,
    callback
  ) {
    return this.connectionPool.query(
      this.countSqlForFetchRequest(request),
      (function(_this) {
        return function(err, result) {
          ...
          return callback(
            ...
            Number(
              result != null
                ? (ref = result[0]) != null ? ref.count : void 0
                : void 0
            )
          );
        };
  ...
  GenericSQLStore.prototype.updateQueryForUpdatedObject = function(
    updatedObject
  ) {
    var attribute,
      e,
    ...
    for (key in values) {
      ...
      if (attribute) {
        ...
        updateValues.push(
          this.encodeValueForAttribute(attribute.encode(value), attribute)
        );
      } else {
        updates.push(this.quoteSymbol + key + this.quoteSymbol + " = ?");
  ...
  GenericSQLStore.prototype.processRequest = function(request) {
    ...
    if (!request.fields) {
      ...
      request.fields[ManagedObjectID.idColumnName] =
        this.tableAlias + "." + ManagedObjectID.idColumnName;
    } else {
      allFieldsMark = null;
      ref1 = request.fields;
      for (name in ref1) {
      ...
      if (allFieldsMark) {
        ...
        for (k = 0, len1 = ref2.length; k < len1; k++) {
          attribute = ref2[k];
          request.fields[attribute.name] =
            this.tableAlias + "." + attribute.name;
        }
        request.fields[ManagedObjectID.idColumnName] =
          this.tableAlias + "." + ManagedObjectID.idColumnName;
      }
    ...
    if (request.type !== "fetch") {
      return (request.fields[ManagedObjectID.idColumnName] =
        this.tableAlias + "." + ManagedObjectID.idColumnName);
    }
  };
  ...
  GenericSQLStore.prototype.countSqlForFetchRequest = function(request) {
    ...
    if (request.predicate) {
      let parsed = this.parsePredicate(request.predicate, request);
      if (parsed) query.where(parsed);
    }
    if (request.havingPredicate) {
      let parsed = this.parsePredicate(request.havingPredicate, request);
      query.having(parsed);
    }
    if (request.group) {
  ...
  GenericSQLStore.prototype.sqlForFetchRequest = function(request) {
    ...
    if (request.resultType === FetchRequest.RESULT_TYPE.MANAGED_OBJECTS) {
      query.group(this.tableAlias + "." + ManagedObjectID.idColumnName);
      query.field(
        this.tableAlias +
          "." +
          this.quoteSymbol +
          ManagedObjectID.idColumnName +
          this.quoteSymbol,
        this.quoteSymbol + ManagedObjectID.idColumnName + this.quoteSymbol
      );
      ref = request.entity.getNonTransientAttributes();
      for (j = 0, len = ref.length; j < len; j++) {
        attribute = ref[j];
        query.field(
          this.tableAlias +
            ...
            attribute.name +
            this.quoteSymbol,
          this.quoteSymbol + attribute.name + this.quoteSymbol
        );
      }
      ref1 = request.entity.relationships;
        ...
        if (!relationship.toMany) {
          ...
          query.field(
            this.tableAlias +
              ...
              columnName +
              this.quoteSymbol,
            this.quoteSymbol + columnName + this.quoteSymbol
          );
        }
      }
    } else {
    ...
    if (
      Array.isArray(request.sortDescriptors) &&
      request.sortDescriptors.length > 0
    ) {
  ...
  GenericSQLStore.prototype._getRawTranslatedQueryWithJoins = function(
    query,
    request
  ) {
    var _subkeys,
      ...
      for (fieldName in ref) {
        ...
        if (fieldValue.indexOf(this.tableAlias + "." + fieldName) === -1) {
          sqlString = sqlString.replace(
            new RegExp(this.tableAlias + "." + fieldName, "g"),
            fieldValue
          );
        }
      }
    }
    clearedSQLString = sqlString
      .replace(/\\"/g, "")
      .replace(/"[^"]+"/g, "")
      .replace(/\\'/g, "")
      .replace(/'[^']+'/g, "");
    joinMatches = clearedSQLString.match(
      new RegExp(this.tableAlias + '(\\.[a-zA-Z_"][a-zA-Z0-9_"]*){2,}', "g")
    );
    ...
    leftJoin = (function(_this) {
      return function(subkeys, parentEntity, path) {
        ...
        if (!relation) {
          throw new Error(
            "relation " + parentEntity.name + "=>" + as + " not found"
          );
        }
        ...
        if (!~alreadyJoined.indexOf(subPath)) {
          ...
          if (relation.toMany && inversedRelation.toMany) {
            primaryRelation = _this._relationshipByPriority(
              relation,
              inversedRelation
            );
            inversedRelation = relation.inverseRelationship();
            middleTableName = _this._getMiddleTableNameForManyToManyRelation(
              primaryRelation
            );
            middleTableNameAlias = pathAlias + "__mid";
            if (primaryRelation === relation) {
              query.left_join(
                _this.quoteSymbol + middleTableName + _this.quoteSymbol,
                middleTableNameAlias,
                parentAlias +
                  "." +
                  ManagedObjectID.idColumnName +
              ...
              query.left_join(
                _this.quoteSymbol +
                  _this._formatTableName(relation.destinationEntity.name) +
                  _this.quoteSymbol,
                pathAlias,
                middleTableNameAlias +
                  "." +
            ...
            if (relation.toMany) {
              query.left_join(
                ...
                pathAlias +
                  "." +
                  _.singularize(inversedRelation.name) +
                  "_id" +
                  " = " +
                  parentAlias +
                  "." +
    ...
    replaceNameSorted = Object.keys(replaceNames)
      .sort()
      .reverse();
    sqlString = query.toString();
    if (request != null ? request.fields : void 0) {
    ...
    for (i in replaceNameSorted) {
      sqlString = sqlString.replace(
        new RegExp(
          replaceNameSorted[i].replace(".", "\\.") + '\\.(?![^\\s_]+\\")',
          "g"
        ),
        replaceNames[replaceNameSorted[i]] + "."
      );
      sqlString = sqlString.replace(
        new RegExp(
          replaceNameSorted[i].replace(".", "\\.") + this.quoteSymbol,
          "g"
        ),
        replaceNames[replaceNameSorted[i]] + this.quoteSymbol
      );
    }
    return sqlString;
  };
  ...
  GenericSQLStore.prototype.processQuery = function(query, request) {
    ...
    if (matches) {
      for (j = 0, len = matches.length; j < len; j++) {
        ...
        columnAfter = match.replace(
          /\.([^\.]+)$/g,
          "." + this.quoteSymbol + "$1" + this.quoteSymbol
        );
    ...
    query = query.replace(
      "FROM " + tableName + " " + this.tableAlias,
      "FROM " +
        this.quoteSymbol +
        tableName +
        this.quoteSymbol +
  ...
  GenericSQLStore.prototype._updateRelationsForObject = function(
    transaction,
    object,
    callback
  ) {
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      if (
        relationship.toMany &&
        inversedRelationship.toMany &&
        object._relationChanges &&
        relationship === reflexiveRelationship
      ) {
        addedObjects =
          (ref1 = object._relationChanges) != null
            ? ref1["added_" + relationship.name]
            : void 0;
        if (addedObjects) {
          for (k = 0, len1 = addedObjects.length; k < len1; k++) {
            addedObject = addedObjects[k];
            sqls.push(
              this._insertQueryForManyToMany(relationship, object, addedObject)
            );
          }
        }
        removedObjects =
          (ref2 = object._relationChanges) != null
            ? ref2["removed_" + relationship.name]
            : void 0;
        if (removedObjects) {
          for (l = 0, len2 = removedObjects.length; l < len2; l++) {
            ...
            sql =
              ...
              this.quoteSymbol +
              " = " +
              this._recordIDForObjectID(removedObject.objectID);
            sqls.push(sql);
          }
        }
      }
  ...
  GenericSQLStore.prototype._insertQueryForManyToMany = function(
    relationship,
    object,
    addedObject
  ) {
  ...
  GenericSQLStore.prototype._getMiddleTableNameForManyToManyRelation = function(
    relationship
  ) {
    ...
    return (
      ...
      "_" +
      reflexiveRelationship.name.toLowerCase()
    );
  };

  GenericSQLStore.prototype._valuesWithRelationshipsForObject = function(
    object
  ) {
    ...
    for (j = 0, len = ref1.length; j < len; j++) {
      ...
      if (!relation.toMany) {
        if (
          ...
          if (
            (ref3 = object._relationChanges) != null
              ? ref3[relation.name]
              : void 0
          ) {
            id = this._recordIDForObjectID(
              object._relationChanges[relation.name].objectID
            );
  ...
  GenericSQLStore.prototype.permanentIDsForObjects = function(objects) {
    ...
    for (j = 0, len = objects.length; j < len; j++) {
      object = objects[j];
      ids.push(
        this._permanentIDForRecord(
          object.entity,
          this.permanentIDsCache[object.objectID.toString()]
        )
      );
    }
    return ids;
  ...
  GenericSQLStore.prototype.newObjectID = function(entity, referenceObject) {
    return new ManagedObjectID(
      this.URL + "/" + entity.name + "/t" + referenceObject,
      entity
    );
  };

  GenericSQLStore.prototype._permanentIDForRecord = function(
    entity,
    referenceObject
  ) {
    return new ManagedObjectID(
      this.URL + "/" + entity.name + "/p" + referenceObject,
      entity
    );
  };
  ...
  GenericSQLStore.prototype._relationshipByPriority = function(
    relationship,
    inversedRelationship
  ) {
  ...
  GenericSQLStore.prototype.columnTypeForAttribute = function(attribute) {
    ...
    switch (attribute.persistentType) {
      ...
      case "decimal":
        type =
          "decimal(" +
          (attribute.info.digits || 20) +
          "," +
          (attribute.info.decimals || 5) +
          ")";
        break;
      case "float":
        type = "float";
  ...
  GenericSQLStore.prototype._columnDefinitionForAttribute = function(
    attribute
  ) {
    var definition, type;
  ...
  GenericSQLStore.prototype.encodeValueForAttribute = function(
    value,
    attribute
  ) {
    if (value === null) {
  ...
  GenericSQLStore.prototype.decodeValueForAttribute = function(
    value,
    attribute
  ) {
    if (value === null) {
    ...
    return new Promise(
      (function(_this) {
        return function(resolve, reject) {
          return _this.connectionPool.createTransaction(function(
            err,
            transaction
          ) {
            if (err) {
            ...
            return _this.createSchemaQueries(options, transaction, function(
              err,
              queries
            ) {
              if (err) {
                ...
                return _this._runRawQueriesInSingleTransaction(
                  queries,
                  transaction,
                  function(err, result) {
                    if (err) {
        ...
        return function(resolve, reject) {
          ...
          return _async.forEachSeries(
            ...
            function(err) {
              ...
              return _this._runRawQueriesInSingleTransaction(queries, function(
                err
              ) {
                if (err) {
                  return reject(err);
                ...
                return _async.forEachSeries(
                  migration.scriptsAfter,
                  function(script, cb) {
                    return _this._runMigrationScript(
                      migration.modelTo,
                      script,
                      cb
                    );
  ...
  GenericSQLStore.prototype._runMigrationScript = function(
    model,
    script,
    callback
  ) {
    var context, err, error, persistentStoreCoordinator;
    persistentStoreCoordinator = new PersistentStoreCoordinator(
      model,
      this.storeCoordinator.globals
    );
      ...
      (function(_this) {
        return function(resolve, reject) {
          ...
          query = squel
            ...
            .field("value")
            .where(
              _this.quoteSymbol + "key" + _this.quoteSymbol + " = ?",
              "version"
            )
            .limit(1);
          return _this.connectionPool.query(query.toString(), function(
            err,
            rows
          ) {
            var ref;
  ...
  GenericSQLStore.prototype.createMigrationQueries = function(migration) {
    var addedEntitiesNames,
      ...
      newReflexiveRelationship,
      newReflexiveTableName,
      newRelationship,
      o,
      p,
      q,
    ...
    for (j = 0, len = ref.length; j < len; j++) {
      ...
      switch (change.change) {
        case "+":
          ...
          sqls = sqls.concat(
            this.createEntityQueries(modelTo.getEntity(entityName), false, {
          ...
          for (k = 0, len1 = ref1.length; k < len1; k++) {
            ...
            if (!relationship.toMany) {
              changedRelationshipsSqls = changedRelationshipsSqls.concat(
                this._addRelationshipQueries(
        ...
        case "-":
          ...
          sqls = sqls.concat(
            this._dropEntityQueries(modelFrom.getEntity(entityName))
          );
          break;
        default:
    ...
    for (n = 0, len4 = updatedEntities.length; n < len4; n++) {
      ...
      if (entityFrom) {
        ...
        for (o = 0, len5 = ref5.length; o < len5; o++) {
          attribute = ref5[o];
          change =
            (ref6 = migration.attributesChanges[entityName]) != null
              ? ref6[attribute.name]
              : void 0;
          if (change) {
            switch (change) {
      ...
      if (entityTo && indexOf.call(addedEntitiesNames, entityName) < 0) {
        ...
        for (p = 0, len6 = ref7.length; p < len6; p++) {
          attribute = ref7[p];
          change =
            (ref8 = migration.attributesChanges[entityName]) != null
              ? ref8[attribute.name]
              : void 0;
          if (change === "+") {
            sqls.push(
              "ALTER TABLE " +
                ...
                this.quoteSymbol +
                " ADD COLUMN " +
                this._columnDefinitionForAttribute(attribute)
            );
          }
        }
      }
      ...
      if (entityFrom) {
        ...
        for (r = 0, len8 = ref11.length; r < len8; r++) {
          ...
          if (relationship.toMany && inverseRelationship.toMany) {
            change =
              ((ref12 = migration.relationshipsChanges[entityName]) != null
                ? ref12[relationship.name]
                : void 0) ||
              ((ref13 =
                migration.relationshipsChanges[
                  inverseRelationship.entity.name
                ]) != null
                ? ref13[inverseRelationship.name]
                : void 0);
            if (change) {
              switch (change) {
                case "+":
        ...
        for (s = 0, len9 = ref14.length; s < len9; s++) {
          ...
          if (relationship.toMany && inverseRelationship.toMany) {
            change =
              (ref15 = migration.relationshipsChanges[entityName]) != null
                ? ref15[relationship.name]
                : void 0;
            if (change && (change !== "+" && change !== "-")) {
              newRelationship = entityTo.getRelationship(change);
              newInverseRelationship = newRelationship.inverseRelationship();
              newReflexiveRelationship = this._relationshipByPriority(
                newRelationship,
                newInverseRelationship
              );
              reflexiveRelationship = this._relationshipByPriority(
              ...
              newReflexiveTableName =
                this.quoteSymbol +
                this._formatTableName(newReflexiveRelationship.entity.name) +
                "_" +
                newReflexiveRelationship.name +
                this.quoteSymbol;
              sqls.push(
                "ALTER TABLE " +
                  reflexiveTableName +
    ...
    for (entityName in ref16) {
      ...
      for (t = 0, len10 = ref17.length; t < len10; t++) {
        ...
        if (!relationship.toMany) {
          change =
            ((ref18 = migration.relationshipsChanges[entityName]) != null
              ? ref18[relationship.name]
              : void 0) ||
            ((ref19 =
              migration.relationshipsChanges[
                inverseRelationship.entity.name
              ]) != null
              ? ref19[inverseRelationship.name]
              : void 0);
          if (change) {
            switch (change) {
              case "+":
    ...
    for (entityName in ref20) {
      ...
      if (
        entityTo &&
        ((ref21 = entityTo.name), indexOf.call(addedEntitiesNames, ref21) < 0)
      ) {
        ...
        for (u = 0, len11 = ref22.length; u < len11; u++) {
          ...
          if (!relationship.toMany) {
            change =
              ((ref23 = migration.relationshipsChanges[entityName]) != null
                ? ref23[relationship.name]
                : void 0) ||
              ((ref24 =
                migration.relationshipsChanges[
                  inverseRelationship.entity.name
                ]) != null
                ? ref24[inverseRelationship.name]
                : void 0);
            switch (change) {
              case "+":
                changedRelationshipsSqls = changedRelationshipsSqls.concat(
      ...
      for (v = 0, len12 = ref25.length; v < len12; v++) {
        ...
        if (relationship.toMany && inverseRelationship.toMany) {
          change =
            ((ref26 = migration.relationshipsChanges[entityName]) != null
              ? ref26[relationship.name]
              : void 0) ||
            ((ref27 =
              migration.relationshipsChanges[
                inverseRelationship.entity.name
              ]) != null
              ? ref27[inverseRelationship.name]
              : void 0);
          if (change === "+") {
            changedRelationshipsSqls = changedRelationshipsSqls.concat(
              this.createRelationshipQueries(relationship)
            );
          }
        }
      }
  ...
  GenericSQLStore.prototype._dropTableQuery = function(tableName) {
    return (
      "DROP TABLE IF EXISTS " + this.quoteSymbol + tableName + this.quoteSymbol
    );
  };
  ...
  GenericSQLStore.prototype._renameRelationshipQuery = function(
    tableName,
    relationshipFrom,
    relationshipTo
  ) {
  ...
  GenericSQLStore.prototype._renameAttributeQuery = function(
    tableName,
    attributeFrom,
    attributeTo
  ) {
  ...
  GenericSQLStore.prototype._removeRelationshipQuery = function(
    entityName,
    relationship
  ) {
  ...
  GenericSQLStore.prototype._addRelationshipQueries = function(
    entityName,
    relationship
  ) {
    return [
      "ALTER TABLE " +
        ...
        this.quoteSymbol +
        " ADD COLUMN " +
        this._relationshipColumnDefinition(relationship)
    ];
  };

  GenericSQLStore.prototype.createEntityRelationshipQueries = function(
    entity,
    force
  ) {
  ...
  GenericSQLStore.prototype.createRelationshipQueries = function(
    relationship,
    force
  ) {
      ...
      if (inverseRelationship.toMany) {
        ...
        sqls.push(
          "CREATE TABLE IF NOT EXISTS `" +
            ...
            "` (`" +
            reflexiveRelationship.name +
            "_id` int(11) NOT NULL,`reflexive` int(11) NOT NULL, PRIMARY KEY (`" +
            reflexiveRelationship.name +
            "_id`,`reflexive`))"
        );
      }
  ...
  GenericSQLStore.prototype._relationshipColumnDefinition = function(
    relationship
  ) {
    return (
      this.quoteSymbol +
  ...
  GenericSQLStore.prototype._runRawQueriesInSingleTransaction = function(
    sqls,
    transaction,
    callback
  ) {
    ...
    return this.connectionPool.createTransaction(
      (function(_this) {
        return function(err, transaction) {
          if (err) {
            return callback(err);
