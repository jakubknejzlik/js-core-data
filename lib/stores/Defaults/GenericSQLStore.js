...
(function() {
  var FetchRequest, GenericPool, GenericSQLStore, IncrementalStore, ManagedObjectID, PersistentStoreRequest, Predicate, SortDescriptor, _, async, squel,
  ...
  GenericSQLStore = (function(superClass) {
            ...
            return async.series([
              function(seriesCallback) {
                return async.forEach(request.insertedObjects, function(insertedObject, cb) {
                  ...
                  sql = 'INSERT INTO ' + formattedTableName + ' (' + _this.quoteSymbol + '_id' + _this.quoteSymbol + ') VALUES (?)';
    ...
    GenericSQLStore.prototype.updateQueryForUpdatedObject = function(updatedObject) {
      ...
      for (key in values) {
        ...
        if (attribute) {
          ...
          updateValues.push(attribute.encode(this.encodeValueForAttribute(value, attribute)));
        } else {
          updates.push(this.quoteSymbol + key + this.quoteSymbol + ' = ?');
    ...
    GenericSQLStore.prototype.sqlForFetchRequest = function(request) {
      var attribute, column, columnName, descriptor, descriptors, field, j, k, l, len, len1, len2, name, query, ref, ref1, ref2, relationship;
      query = squel.select().from(this._formatTableName(request.entity.name), this.tableAlias);
      if (request.resultType === FetchRequest.RESULT_TYPE.MANAGED_OBJECTS) {
        query.group('SELF._id');
        query.field(this.tableAlias + '.' + this.quoteSymbol + '_id' + this.quoteSymbol, '_id');
        ref = request.entity.attributes;
        for (j = 0, len = ref.length; j < len; j++) {
          attribute = ref[j];
          query.field(this.tableAlias + '.' + this.quoteSymbol + attribute.name + this.quoteSymbol, attribute.name);
        }
        ref1 = request.entity.relationships;
        for (k = 0, len1 = ref1.length; k < len1; k++) {
          ...
          if (!relationship.toMany) {
            columnName = _.singularize(relationship.name) + '_id';
            query.field(this.tableAlias + '.' + this.quoteSymbol + columnName + this.quoteSymbol, columnName);
          }
        }
      } else {
        if (!request.fields) {
    ...
    GenericSQLStore.prototype._valuesWithRelationshipsForObject = function(object) {
      ...
      for (key in ref) {
        ...
        data[key] = this.decodeValueForAttribute(value, attribute);
      }
      ...
      return this.getCurrentVersion((function(_this) {
        return function(err, currentVersion) {
          ...
          if (currentVersion === objectModel.version && !options.force) {
            ...
            return _this._runRawQueriesInTransaction(queries, callback);
          } else {
    ...
    GenericSQLStore.prototype.getCurrentVersion = function(callback) {
      ...
      return this.connection.sendRawQuery(query.toString(), function(err, rows) {
        var ref;
        if (err) {
          return callback(err);
        }
    ...
    GenericSQLStore.prototype.createMigrationQueries = function(migration) {
      var attribute, change, e, entityFrom, entityName, entityTo, inversedRelationship, j, k, l, len, len1, len2, len3, len4, m, modelFrom, modelTo, n, newColumnNames, oldAttribute, oldColumnNames, oldInversedRelationship, oldReflexiveRelationship, oldReflexiveTableName, oldRelationship, ref, ref1, ref2, ref3, reflexiveRelationship, reflexiveTableName, relationship, sqls, tableName, updatedEntities;
      sqls = [];
      ...
      for (k = 0, len1 = updatedEntities.length; k < len1; k++) {
        ...
        tableName = this._formatTableName(entityName);
        ...
        if (relationship.toMany && inversedRelationship.toMany) {
          ...
          if (change) {
            if (change !== '+' && change !== '-') {
              ...
              reflexiveRelationship = this._relationshipByPriority(relationship, inversedRelationship);
              oldReflexiveTableName = this._formatTableName(oldReflexiveRelationship.entity.name) + '_' + oldReflexiveRelationship.name;
            ...
            sqls.push(this.createEntityRelationshipQueries(entityTo));
          }
        }
      }
      return sqls;
    ...
    GenericSQLStore.prototype._runRawQueriesInTransaction = function(sqls, callback) {
      return this.connection.createTransaction((function(_this) {
        return function(transaction) {
          return async.forEachSeries(sqls, function(sql, cb) {
            return transaction.sendQuery(sql, cb);
          }, function(err) {
            if (err) {
              return transaction.rollback(function() {
                ...
                return _this.connection.releaseTransaction(transaction);
              });
            } else {
